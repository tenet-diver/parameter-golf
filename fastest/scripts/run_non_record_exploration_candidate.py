...
def select_next_non_record_exploration_candidate(tasks: list[dict], lane: str = "non-record-exploration") -> dict | None:
    eligible: list[dict] = []
    for task in tasks:
...
def _max_non_record_exploration_experiments(task: dict) -> int:
    budget_caps = task.get("budgetCaps")
    if not isinstance(budget_caps, dict):
        return 1
    value = budget_caps.get("maxNonRecordExplorationExperiments")
...
def run_next_non_record_exploration_candidate(
    tasks: list[dict],
    *,
    runner: Runner,
    lane: str = "non-record-exploration",
    ...
    if lane != "non-record-exploration":
        return _outcome(
            ...
            message="Task lane must be non-record-exploration.",
        )

    selected = select_next_non_record_exploration_candidate(tasks, lane=lane)
    if selected is None:
        return _outcome(
            task={"lane": lane},
            status="success",
            reason_code="no-task-available",
            message="No runnable non-record exploration candidate is available.",
        )
    ...
    if not isinstance(selected.get("taskId"), str) or not selected.get("taskId"):
        return _outcome(
            ...
            message="Selected non-record exploration task is missing taskId.",
        )
    if not isinstance(selected.get("candidateId"), str) or not selected.get("candidateId"):
        return _outcome(
            ...
            status="error",
            reason_code="task-io-invalid",
            message="Selected non-record exploration task is missing candidateId.",
        )

    return execute_non_record_exploration_candidate(
        selected,
        runner=runner,
...
def execute_non_record_exploration_candidate(
    task: dict,
    *,
    runner: Runner,
    evidence_path: Path = DEFAULT_EVIDENCE_PATH,
    ...
    if trusted_control_state not in {"established", "verified"}:
        return _outcome(
            ...
            message="Trusted control baseline is not established; skip non-record exploration until baseline evidence is verified.",
        )
    ...
    existing = next(
        (
            ...
            and (
                ...
                or (
                    record.get("lane") == "non-record-exploration"
    ...
    used_combination_experiments = sum(
        ...
        for record in experiment_records
        if isinstance(record, dict)
        and record.get("lane") == "non-record-exploration"
    )
    if used_combination_experiments >= max_combination_experiments:
        return _outcome(
            task=task,
            status="error",
            reason_code="non-record-exploration-budget-exhausted",
            message="Non-record exploration experiment budget is exhausted for this run configuration.",
        )
    ...
    try:
        ...
        return _outcome(
            ...
            message=f"Non-record exploration runner failed: {exc.__class__.__name__}: {exc}",
        )
...
def _default_runner(task: dict) -> dict:
    ...
    if not isinstance(candidate_id, str) or not candidate_id:
        raise ValueError("candidateId is required for non-record exploration execution")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Execute one bounded non-record exploration benchmark candidate.")
    parser.add_argument("--task", help="Path to non-record-exploration task JSON file.")
    parser.add_argument(
        "--task-store-dir",
    ...
    if args.task:
        try:
            ...
            outcome = execute_non_record_exploration_candidate(
                task_payload,
