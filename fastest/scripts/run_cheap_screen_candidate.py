...
def _synthesize_candidate_payload(task_id: str, lane: str, modules: list) -> dict:
    ...
    return {
        "lane": lane,
        "candidateId": f"exp-{task_fragment}-cheap-screen-001",
        "traceId": f"trace-{task_fragment}-cheap-screen-001",
...
def _load_tasks_from_task_store(task_store_dir: Path) -> list[dict]:
    ...
    for row in rows:
        ...
        if not payload:
            payload = _synthesize_candidate_payload(task_id, "cheap-screen", modules)
        task = {
            "taskId": task_id,
            "status": status,
