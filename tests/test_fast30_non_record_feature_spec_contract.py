...
FEATURE_SPEC_PATH = REPO_ROOT / "planning/non-record-exploration-feature-spec-fast30.json"


class Fast30NonRecordFeatureSpecContractTest(unittest.TestCase):
    def test_feature_spec_satisfies_fast30_design_first_contract(self) -> None:
        spec = json.loads(FEATURE_SPEC_PATH.read_text())

        self.assertEqual(spec["taskId"], "FAST-30")
        self.assertEqual(spec["title"], "Explore one non-record benchmark direction for parameter-golf")
        ...
        self.assertTrue(
            any(
                "maxexperimentsperdirection" in entry.lower()
