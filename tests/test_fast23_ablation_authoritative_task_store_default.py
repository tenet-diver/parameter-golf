...
class Fast23AblationAuthoritativeTaskStoreDefaultTest(unittest.TestCase):
    def test_default_task_store_dir_honors_task_store_env_override(self) -> None:
        ...
        try:
            with mock.patch.dict(
                os.environ, {"TASK_STORE_DIR": str(AUTHORITATIVE_TASK_STORE_DIR)}
            ):
                reloaded = importlib.reload(ablation_module)
            ...
            importlib.reload(ablation_module)
    ...
    def test_task_store_load_uses_explicit_read_only_sqlite_uri(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        json.dumps(
                            {
                                "lane": "ablation",
                                "candidateId": "exp-fast23-001",
                                "traceId": "trace-fast23-001",
                            }
                        ),
                    ),
                )
            ...
            with mock.patch.object(ablation_module.sqlite3, "connect", side_effect=track_connect):
