...
class Fast26MeasurementCoverageRefreshTest(unittest.TestCase):
    def test_authoritative_evidence_contains_reproducible_fast26_measurement_record(self) -> None:
        source = json.loads(SOURCE_PATH.read_text())
        records = source["experimentRecords"]
        fast26_records = [
            record
            for record in records
            if record.get("taskId") == "FAST-26"
            and record.get("lane") == "measurement"
            and record.get("status") == "accepted"
        ]
        self.assertTrue(
            fast26_records,
            "Expected at least one accepted FAST-26 measurement record in authoritative evidence.",
        )

        record = fast26_records[-1]
