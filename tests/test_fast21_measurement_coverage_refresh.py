...
class Fast21MeasurementCoverageRefreshTest(unittest.TestCase):
    def test_authoritative_evidence_contains_reproducible_fast21_measurement_record(self) -> None:
        source = json.loads(SOURCE_PATH.read_text())
        records = source["experimentRecords"]
        fast21_records = [
            record
            for record in records
            if record.get("taskId") == "FAST-21"
            and record.get("lane") == "measurement"
            and record.get("status") == "accepted"
        ]
        self.assertTrue(
            fast21_records,
            "Expected at least one accepted FAST-21 measurement record in authoritative evidence.",
        )

        record = fast21_records[-1]
